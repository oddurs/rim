...
//! reference match them. `RIM_UPDATE_TYPES=1` rewrites the files.
...
fn every_rim_member_is_declared_and_the_generated_files_are_current() {
    ...
    for (file, text) in
        [("types/rim.d.luau", s.scripts.luau_definitions()), ("docs/modding/api-scripts.md", s.scripts.api_reference())]
    {
