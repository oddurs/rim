...
From 0144 (patch list operations): the loader's PatchLog records set_by (field paths) and list_by (lists appended to, removed from or edited, including matched elements as list[key=value]). The report should list those list edits alongside patched fields; 0144's last criterion leaves that half to this item.
